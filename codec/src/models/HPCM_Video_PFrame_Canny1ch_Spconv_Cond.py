...
M = HPCM_Canny1ch_Spconv_Cond.LATENT_M
...
class HPCM(nn.Module):
    def __init__(self, use_lossy_ref: bool = True):
        ...
        self.iframe_codec = HPCM_Canny1ch_Spconv_Cond.HPCM()
        self.codec = HPCM_Canny1ch_Spconv_Cond.HPCM()
    ...
    def _to_1ch(x: torch.Tensor) -> torch.Tensor:
        return x[:, :1] if x.size(1) > 1 else x

    @staticmethod
    def _curr_canny(p_input: torch.Tensor) -> torch.Tensor:
        return p_input if p_input.size(1) == 1 else p_input[:, 2:3]

    def _merge_temporal_context(self, params: torch.Tensor, ref_feats: dict) -> torch.Tensor:
    ...
    @torch.no_grad()
    def compress_p(self, x_pad: torch.Tensor, ref_feats: dict, cond: torch.Tensor | None = None) -> dict:
    ...
    @torch.no_grad()
    def decompress_p(self, strings, shape, ref_feats: dict, cond: torch.Tensor | None = None) -> dict:
    ...
    @torch.no_grad()
    def compress_i(self, x_pad: torch.Tensor, cond: torch.Tensor | None = None) -> dict:
        return self.iframe_codec.compress(x_pad, cond=cond)

    @torch.no_grad()
    def decompress_i(self, strings, shape, cond: torch.Tensor | None = None) -> dict:
        out = self.iframe_codec.decompress_with_latent(strings, shape, cond=cond)
