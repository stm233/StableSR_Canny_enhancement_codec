"""P-frame video codec on HPCM_Canny1ch: temporal f1-f3 from prev decoder, f4 -> context only.

Encoder input is current-frame 1ch Canny; prev lossy I-frame supplies ref_feats + optional
explicit prev_hat is not concatenated (temporal via multi-scale fusion only).
"""
...
M = HPCM_Canny1ch.LATENT_M
C32, C64, C128 = 32, 64, 128
CTX_CH = M * 2


class HPCM(nn.Module):
    """DCVC-style P-frame for Canny1ch I-frame codec (M=N=128)."""
    ...
    def __init__(self, use_lossy_ref: bool = True):
        ...
        self.iframe_codec = HPCM_Canny1ch.HPCM()
        self.codec = HPCM_Canny1ch.HPCM()
    ...
    def _curr_canny(p_input: torch.Tensor) -> torch.Tensor:
        """PFrameDataset packs [prev, prev, curr] as 3ch; use curr (B channel)."""
        if p_input.size(1) == 1:
            return p_input
        return p_input[:, 2:3]
