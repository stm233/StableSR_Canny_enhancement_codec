"""HPCM Canny1ch with MinkowskiEngine convolutions in g_a/g_s/h_a/h_s (ablation variant).

Dense entropy / spatial-prior path unchanged. Checkpoint weights are NOT compatible
with HPCM_Canny1ch (dense conv).
"""
...
from src.layers.me_conv import (
    me_conv2x2_down,
    me_conv4x4_down,
    me_deconv2x2_up,
    me_deconv4x4_up,
)
from src.layers.me_res_blk import MEPConvRB

__all__ = ["HPCM", "LATENT_M", "LATENT_N", "g_a_1ch_me", "g_s_1ch_me", "h_a_128_me", "h_s_128_me"]


class g_a_1ch_me(nn.Module):
    """ME encoder: 1 -> 32 -> 64 -> 128 -> M (same topology as g_a_1ch)."""
    ...
    def __init__(self, m: int = LATENT_M):
        ...
        self.branch = nn.Sequential(
            me_conv4x4_down(1, c32),
            MEPConvRB(c32, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c32, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            me_conv2x2_down(c32, c64),
            MEPConvRB(c64, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c64, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            me_conv2x2_down(c64, c128),
            MEPConvRB(c128, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c128, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c128, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c128, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            me_conv2x2_down(c128, m),
        )
...
class g_s_1ch_me(nn.Module):
    """ME decoder: M -> 128 -> 64 -> 32 -> 1."""
    ...
    def __init__(self, m: int = LATENT_M):
        ...
        c32, c64, c128 = 32, 64, 128
        self.branch = nn.Sequential(
            me_deconv2x2_up(m, c128),
            MEPConvRB(c128, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c128, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c128, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c128, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            me_deconv2x2_up(c128, c64),
            MEPConvRB(c64, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c64, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            me_deconv2x2_up(c64, c32),
            MEPConvRB(c32, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(c32, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            me_deconv4x4_up(c32, 1),
        )
...
class h_a_128_me(nn.Module):
    """ME hyper encoder: M -> N."""
    ...
    def __init__(self, m: int = LATENT_M, n: int = LATENT_N):
        ...
        partial_ratio = 4
        self.branch = nn.Sequential(
            MEPConvRB(m, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            me_conv2x2_down(m, n),
            MEPConvRB(n, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(n, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(n, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            me_conv2x2_down(n, n),
        )
...
class h_s_128_me(nn.Module):
    """ME hyper decoder: N -> 2*M."""
    ...
    def __init__(self, m: int = LATENT_M, n: int = LATENT_N):
        ...
        self.branch = nn.Sequential(
            ...
            MEPConvRB(n, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            MEPConvRB(n, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
            me_deconv2x2_up(n, m * 2),
            MEPConvRB(m * 2, mlp_ratio=mlp_ratio, partial_ratio=partial_ratio),
        )
...
class HPCM(basemodel):
    """1ch Canny codec; encoder/decoder use MinkowskiEngine convolutions."""
    ...
    def __init__(self, m: int = LATENT_M, n: int = LATENT_N):
        ...
        self.g_a = g_a_1ch_me(m)
        self.g_s = g_s_1ch_me(m)
        self.h_a = h_a_128_me(m, n)
        self.h_s = h_s_128_me(m, n)
