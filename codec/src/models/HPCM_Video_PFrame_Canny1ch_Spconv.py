"""P-frame video codec on HPCM_Canny1ch_Spconv (spconv encoder/decoder).

Ablation variant: iframe_codec + trainable codec use spconv conv in g_a/g_s/h_a/h_s.
...
from . import HPCM_Canny1ch_Spconv
...
M = HPCM_Canny1ch_Spconv.LATENT_M
C32, C64, C128 = 32, 64, 128
CTX_CH = M * 2


class HPCM(nn.Module):
    """DCVC-style P-frame for Canny1ch spconv I-frame codec (M=N=128)."""
    ...
    def __init__(self, use_lossy_ref: bool = True):
        ...
        self.iframe_codec = HPCM_Canny1ch_Spconv.HPCM()
        self.codec = HPCM_Canny1ch_Spconv.HPCM()
